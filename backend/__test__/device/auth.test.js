...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'password':'GMc0eL_NbSaNkXT',
        'email':'Dedric_Kohler@yahoo.com',
        'name':'Kellie Hane',
        'userType':authConstant.USER_TYPES.User,
        'phone_number':'monitor',
        'profile_img':'Cambridgeshire',
        'sign_img':'cross-platform',
        'address_line_1':'Hawaii',
        'address_line_2':'Rand',
        'crater_name':'Health',
        'colony_name':'Shoes',
        'pincode':'mission-critical',
        'age':72,
        'profession':'Intelligent',
        'experience':'Manat',
        'dob':'2023-06-07T03:38:15.861Z',
        'hobbies':'Bacon',
        'SDGs':'Colombian',
        'investment_frequency':'utilize',
        'company_domains':'platforms',
        'impact_domains':'olive',
        'notif_token':'Fall',
        'isAuth':false,
        'blob':'National',
        'mobileNo':'(218) 450-2166'
      });
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    let user = await request(app)
      .post('/device/auth/forgot-password')
      .send({ 'email':'Dedric_Kohler@yahoo.com', });
...
describe('POST /validate-otp -> otp is sent in request body and OTP is correct', () => {
  test('should return success', () => {
    return request(app)
      ...
      .send(
        {
          username: 'Dedric_Kohler@yahoo.com',
          password: 'GMc0eL_NbSaNkXT'
        }).then(login => () => {
        return request(app)
