...
export function BottomSheet({
  ...
  const [RiskVal, setRiskVal] = useState<any>("");
  const [MarketCapVal, setMarketCapVal] = useState<any>("");
  const [CountryVal, setCountryVal] = useState<string[]>([]);
  const [CompAge, setCompAge] = useState<any>("");
  const [InvstFreqVal, setInvstFreqVal] = useState<any>("");
  ...
  const filterList = {
    ...
    "Investment Options": <Inv_Opt Checked={InvOption} setChecked={setInvOption}/>,
    "Company Size": <Company_size Checked={CompanySize} setChecked={setCompanySize} />,
    "Risk": <Risk Checked={RiskVal} setChecked={setRiskVal} />,
    // TODO
    "Market Cap": <Market_cap Checked={MarketCapVal} setChecked={setMarketCapVal} />,
    "Country": <Country Checked={CountryVal} setChecked={setCountryVal}/>,
    "Age Of Company": <Age_Of_Company Checked={CompAge} setChecked={setCompAge} />,
    "Investment Frequency": <Inv_time Checked={InvstFreqVal} setChecked={setInvstFreqVal} />,
    // TODO
    // "SDG aimed": <Sdg_aimed Checked={CompAge} setChecked={setCompAge} />,
    // TODO
    "Company Industry": <Company_ind Checked={CompIndstryVal} setChecked={setCompIndstryVal} />,
    // TODO
    "Minimum Amt": <Min_amt />,
    "Impact Domain": <Impact_dom Checked={ImpactDomainVal} setChecked={setImpactDomainVal} />,
  };
  ...
  return (
    <>
      ...
      {show && (
        ...
        >
          <View style={{backgroundColor: '#4e4e4e', height: 64, width: 64, alignSelf: 'center', top: screenHeight*0.24, borderRadius: 40, alignItems: 'center', justifyContent: 'center'}}>
      ...
      <Animated.View style={[styles.bottomSheet, { height, bottom }]}>
        ...
        <View style={{ flexDirection: "row", flex: 1,borderTopWidth: 0, borderTopColor: 'grey' }}>
          <View style={{ flex: 0.35, backgroundColor: "#1e1e1e", paddingRight: 4 }}>
            <FlatList
                ...
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    SelectedFilter == val.item
                      ? { backgroundColor: "#2e2e2e", borderWidth: 1, borderColor: '#3e3e3e', borderRadius: 8, margin: 2 }
                      : {},
                  ]}
                >
                  <H3 custom_style={[
          ...
          <View style={{ flex: 0.75, backgroundColor: "#1e1e1e", paddingLeft: 4 }}>
            {/* @ts-ignore */}
            {SelectedFilter && filterList[SelectedFilter]}
          </View>
        </View>
        <View style={{backgroundColor: '#1e1e1e', width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 8, borderTopWidth: 0.4, borderTopColor: '#5e5e5e'}}>
          <TouchableOpacity style={{ width: '49%', padding: 8, borderRadius: 8}}>
            <Text style={{fontFamily: 'm', color: '#dedede', textAlign: 'center'}}>Reset Filters</Text>
          </TouchableOpacity>

          <TouchableOpacity style={{ backgroundColor: '#308CF6', width: '49%', padding: 8, alignItems:'center', justifyContent: 'center', borderRadius: 16, height: 48 }}>
            <Text style={{ fontFamily: 'mm', color: '#fefefe', fontSize: 16 }}>Apply</Text>
          </TouchableOpacity>
        </View>
      </Animated.View>
    </>
